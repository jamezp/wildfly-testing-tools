...
`wildfly.http.protocol`::
Protocol for HTTP connections. Default: `http`.

`wildfly.http.host`::
Host for HTTP connections. Default: `localhost`.

`wildfly.http.port`::
Port for HTTP connections. Default: `8080`.
...
When default ports are unavailable:

[source,bash]
----
mvn test \
  -Dwildfly.http.port=8180 \
  -Dwildfly.management.port=10090
----
...
[source,properties]
----
wildfly.http.port=8180
wildfly.management.port=10090
----
...
[source,bash]
----
mvn test -Dwildfly.jvm.args="-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=8787"
----
